...
_(Alternatively, run `npm install --prefix backend` and `npm install --prefix frontend`)_
...
Fetch paginated employees with optional filtering and sorting. _(Admins view all; regular employees view self)._
...
Fetch a specific employee by ID. _(Admins can view any; employees can only view self)._
...
Update employee details. _(Admins can update all fields; employees can only update their `name`)._
...
#### 4. `deleteEmployee` _(Admin Only)_
...
- **Standard Seed (Safe Upsert):**
  ...
  _Safe to rerun multiple times without duplicating or dropping existing data._
...
_(Automated unit and integration test suites like Jest or Vitest are not currently configured in the codebase)._
...
- **Jay Khatri** - [khatrishyam0987@gmail.com](mailto:khatrishyam0987@gmail.com)
